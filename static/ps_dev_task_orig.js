 ...
 var startTask = function () {
	...
	var endTime = new Date().toISOString().substr(0, 19);
     	$.ajax("quitter", {
                type: "POST",
                async: false,
                data: {prolific_id: prolific_id,study_id:study_id, participant_id:participant_id, datastring: datastring,when:endTime}
                });
    ...
    kd.stop(); //to stop the kd run that was ran earlier (no keyboard response from now on!)
	  removeListener(); //this is so that you can type in the medlist
		...
		payment = payment.concat("\t", bonusPayFin);
			...
			type: "POST",
			async: false,
			data:  {participant_id: participant_id,study_id:study_id, prolific_id:prolific_id, datastring: datastring, payment:payment, when:endTime},
		});
			return false;
...
var toRavens = function(){
    showpage('q_ravens');
	 ...
	 // console.log(questDatastring);
		 ...
		 type: "POST",
		 async: false,
		 data: {workerId: workerId,assignmentId:assignmentId, hitId:hitId, questDatastring: questDatastring, when:endTime},
	 });
	return false;
};
